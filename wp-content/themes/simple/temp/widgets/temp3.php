<?php
/*閲覧履歴　ウィジェット */
class history_Widget extends WP_Widget {
function history_Widget() {
parent::WP_Widget(
false,
$name = '閲覧履歴',
array( 'description' => '閲覧履歴を表示します。', )
);
}
...
echo $before_widget;
if( $title ){echo $before_title . $title . $after_title;}else{echo $before_title . '閲覧履歴' . $after_title;}
?>
<?php 
...
//履歴の有無
if($COOKIE_data = esc_sql($_COOKIE['single'])):
?>
<?php krsort($COOKIE_data);$the_query = new WP_Query(array('post__in' => $COOKIE_data,'orderby' => 'post__in','post_status' => 'publish','posts_per_page' => wp_filter_nohtml_kses($instance['limit'])));if ( $the_query->have_posts() ) : while ( $the_query->have_posts() ) : $the_query->the_post(); ?><?php if($instance['show_img']):?><div class="img-new-post clearfix"><div class="new-post-img<?php echo wp_filter_nohtml_kses(wp_filter_nohtml_kses($show_position));?>"><?php if( has_post_thumbnail() ): ?><a href="<?php echo get_permalink(); ?>"><?php echo get_the_post_thumbnail($the_query->post->ID,$show_shape); ?></a><?php else: ?><a href="<?php echo get_permalink(); ?>"><img src="<?php bloginfo('template_url'); ?>/img/no-<?php echo wp_filter_nohtml_kses($show_shape) ?>.png" alt="no-image"></a><?php endif; ?><?php if($show_view):?><div class="list-post-view"><span class="post-view-count"><?php echo number_format((int)esc_html(get_post_meta($the_query->post->ID,"view",true))); ?></span> <span class="post-views">views</span></div><?php endif; ?></div><div class="new-post-title"><p class="wideget-title-color"><a href="<?php echo esc_url(get_permalink()); ?>"><?php the_title(); ?></a></p><?php if($show_list_img):?><?php if($show_category):?><p class="list-category-op"><?php the_category(' ','',$the_query->post->ID); ?></p><?php endif; ?><p class="wideget-post-op"><?php if($show_date):?><span class="list-category-date"><?php echo get_the_date(); ?></span><?php endif; ?><?php if($show_comments):?><span class="list-category-comments"><?php echo number_format(get_comments_number()); ?> 件</span><?php endif; ?><?php if($show_author):?><span class="list-category-author"><?php the_author_posts_link(); ?></span><?php endif; ?></p><?php endif; ?></div></div><?php else: ?>
<div class="new-post-text">
<p class="wideget-title-color">
...
<?php echo number_format((int)esc_html(get_post_meta($the_query->post->ID,"view",true))); ?> views</span><?php endif; ?>
...
<?php else : ?>
<!-- 履歴の有無 -->
<div class="img-new-post clearfix no-post">
現在履歴はありません。
</div>
<?php endif?>


<?php echo $after_widget;}}register_widget('history_Widget');?>